...
func (s *AuthService) Login(ctx context.Context, email, password string) (accessToken, refreshToken string, err error) {
	// 1. 從資料庫撈取使用者
	user, err := database.UserGetByEmail(email)
	if err != nil {
		return "", "", err
	}
	s.email = user.Email
	...
	s.role = user.Role
	s.group = user.Group
	s.emailVerify = user.Email_check


	// 2. 驗證密碼
	_, status, err := database.UserLoginCheck(email, password)
	if err != nil {
		return "", "", err
	} else {
		if !status {
	...
	// 3. 產生 Access Token
	accessToken, err = s.generateToken(user.Email, user.Role, s.config.AccessExpiry, s.config.AccessSecret)
	if err != nil {
		return "", "", err
	}

	// 4. 產生 Refresh Token
	refreshToken, err = s.generateToken(user.Email, user.Role, s.config.RefreshExpiry, s.config.RefreshSecret)
	if err != nil {
		return "", "", err
	}
...
func (s *AuthService) Refresh(ctx context.Context, refreshTokenString string) (newAccessToken string, err error) {
	...
	// 2. (可選) 檢查使用者是否仍然存在/有效
	_, err = database.UserGetByEmail(claims.UserID)
	if err != nil {
...
func (s *AuthService) generateToken(userID string, role string, expiry time.Duration, secret string) (string, error) {
	claims := CustomClaims{
		UserID: userID,
		Role:   role,
